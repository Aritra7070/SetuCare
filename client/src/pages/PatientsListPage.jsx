...
export const PatientsListPage = ({ onNavigateToRegister }) => {
  ...
  const [editFormData,   setEditFormData]   = useState({
    name: '', dob: '', gender: 'female', guardianName: '', phone: '', address: '', preferredLanguage: 'mr',
  });
  ...
  useEffect(() => { fetchPatients(); }, [selectedGender]);

  const handleSearchSubmit = (e) => { e.preventDefault(); fetchPatients(); };
    ...
    <div className="main-content">
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem', marginBottom: '1.5rem' }}>
        ...
        <button onClick={onNavigateToRegister} className="btn btn-primary btn-sm">
          <Plus size={15} /> {t('patients.registerNew')}
        </button>
      </div>
      ...
      <div className="card" style={{ padding: '1rem 1.25rem', marginBottom: '1.5rem', display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: '1rem' }}>
        <form onSubmit={handleSearchSubmit} style={{ display: 'flex', gap: '0.5rem', flex: '1 1 280px' }}>
          ...
          <button type="submit" className="btn btn-outline btn-sm">{t('patients.search')}</button>
        </form>

        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          <select className="form-select" style={{ width: 'auto', padding: '0.5rem 0.8rem', fontSize: '0.85rem' }} value={selectedGender} onChange={(e) => setSelectedGender(e.target.value)}>
          ...
          <button onClick={fetchPatients} className="btn btn-outline btn-sm" title={t('common.refresh')}>
            <RefreshCw size={14} />
          </button>
        </div>
      </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '0.875rem' }}>
            <thead>
              <tr style={{ background: 'rgba(15,23,42,0.9)', borderBottom: '1px solid var(--border-subtle)', color: 'var(--text-secondary)' }}>
                <th style={{ padding: '1rem' }}>{t('patients.colNamePhid')}</th>
                <th style={{ padding: '1rem' }}>{t('patients.colAge')}</th>
                <th style={{ padding: '1rem' }}>{t('patients.colGuardian')}</th>
                <th style={{ padding: '1rem' }}>{t('patients.colContact')}</th>
                <th style={{ padding: '1rem' }}>{t('patients.colFacility')}</th>
                <th style={{ padding: '1rem', textAlign: 'right' }}>{t('patients.colActions')}</th>
              </tr>
            </thead>
            <tbody>
              {patients.length === 0 ? (
                  ...
                  <tr key={p._id} style={{ borderBottom: '1px solid var(--border-subtle)' }}>
                    <td style={{ padding: '1rem' }}>
                      ...
                      <div style={{ marginTop: '3px' }}>
                        <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.8rem', color: '#22d3ee', background: 'rgba(6,182,212,0.1)', padding: '0.15rem 0.45rem', borderRadius: '4px', border: '1px solid rgba(6,182,212,0.25)' }}>
                          {p.phid}
                        </span>
                      </div>
                    </td>
                    <td style={{ padding: '1rem' }}>
                      <div style={{ fontWeight: '600', color: '#e2e8f0' }}>{calculateAge(p.dob)}</div>
                    ...
                    <td style={{ padding: '1rem', color: p.guardianName ? '#fde68a' : 'var(--text-muted)' }}>
                      {p.guardianName || t('patients.adultGuardian')}
                    </td>
                    <td style={{ padding: '1rem' }}>
                      <div style={{ color: '#f1f5f9' }}>{p.phone || t('patients.noPhone')}</div>
                      <div style={{ fontSize: '0.78rem', color: 'var(--text-muted)' }}>{p.address || 'Maharashtra'}</div>
                    </td>
                    <td style={{ padding: '1rem' }}>
                      <div style={{ fontWeight: '600', color: '#38bdf8' }}>{p.registeredAtFacility?.name || 'Local Facility'}</div>
                      <div style={{ fontSize: '0.78rem', color: 'var(--text-muted)' }}>
                        {p.registeredAtFacility?.district} &bull;{' '}
                        <span style={{ textTransform: 'uppercase' }}>{p.registeredAtFacility?.tier?.replace('_', ' ')}</span>
                      </div>
                    </td>
                    <td style={{ padding: '1rem', textAlign: 'right' }}>
                      <div style={{ display: 'inline-flex', gap: '0.4rem' }}>
                        <button onClick={() => openCardModal(p._id)} className="btn btn-primary btn-sm" style={{ padding: '0.35rem 0.7rem' }} title="View / Print PHID QR Card" disabled={cardLoading}>
                          <QrCode size={13} /> {t('patients.cardBtn')}
                        </button>
                        <button onClick={() => openEditModal(p)} className="btn btn-outline btn-sm" style={{ padding: '0.35rem 0.6rem' }} title={t('common.edit')}>
                          <Edit2 size={13} />
                        </button>
                      </div>
                    </td>
      ...
      {selectedCardPatient && (
        <PatientCardModal
          ...
          age={cardAge}
          onClose={() => { setSelectedCardPatient(null); setCardQrDataUrl(null); setCardAge(null); }}
        />
      )}
      ...
      {editingPatient && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: '1.5rem' }}>
          <div className="card" style={{ width: '100%', maxWidth: '540px', border: '1px solid var(--border-focus)', boxShadow: '0 20px 50px rgba(0,0,0,0.8)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem' }}>
              <div>
                <h3 style={{ fontSize: '1.25rem', fontWeight: '800' }}>{t('patients.editTitle')}</h3>
                <div style={{ fontSize: '0.8rem', color: '#22d3ee', fontFamily: 'var(--font-mono)' }}>PHID: {editingPatient.phid}</div>
              </div>
              <button onClick={() => setEditingPatient(null)} style={{ background: 'none', color: '#94a3b8' }}><X size={20} /></button>
            </div>

            {editError && (
              <div className="alert alert-error"><AlertTriangle size={16} /><div>{editError}</div></div>
            )}

            <form onSubmit={handleEditSave}>
              <div className="form-group">
                <label className="form-label">{t('patient.fullName')}</label>
                <input type="text" required className="form-input" value={editFormData.name} onChange={(e) => setEditFormData({ ...editFormData, name: e.target.value })} />
              </div>

              <div className="grid-2" style={{ gap: '0.9rem' }}>
                <div className="form-group">
                  <label className="form-label">{t('patient.dob')}</label>
              ...
              <div className="form-group">
                <label className="form-label">{t('patients.guardianMinor')}</label>
                <input type="text" className="form-input" placeholder={t('patients.guardianPlaceholder')} value={editFormData.guardianName} onChange={(e) => setEditFormData({ ...editFormData, guardianName: e.target.value })} />
              </div>

              <div className="grid-2" style={{ gap: '0.9rem' }}>
                <div className="form-group">
                  <label className="form-label">{t('patient.phone')}</label>
                  <input type="tel" className="form-input" value={editFormData.phone} onChange={(e) => setEditFormData({ ...editFormData, phone: e.target.value })} />
                </div>
                <div className="form-group">
                  <label className="form-label">{t('patients.language')}</label>
                  <select className="form-select" value={editFormData.preferredLanguage} onChange={(e) => setEditFormData({ ...editFormData, preferredLanguage: e.target.value })}>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.75rem', marginTop: '1.25rem' }}>
                <button type="button" onClick={() => setEditingPatient(null)} className="btn btn-outline">{t('common.cancel')}</button>
                <button type="submit" disabled={editSaving} className="btn btn-primary">
                  {editSaving ? t('patients.editSaving') : t('patients.editSave')}
                </button>
              </div>
            </form>
          </div>
