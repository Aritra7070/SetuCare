...
// ---------------------------------------------------------------------------
const STATUS_CFG = {
  available: { label: 'Available', bg: 'rgba(16,185,129,0.15)',  color: '#34d399', border: 'rgba(16,185,129,0.3)',  Icon: CheckCircle2 },
  low:       { label: 'Low Stock', bg: 'rgba(245,158,11,0.15)',  color: '#fbbf24', border: 'rgba(245,158,11,0.3)',  Icon: AlertTriangle },
  out:       { label: 'Out',       bg: 'rgba(244,63,94,0.15)',   color: '#fb7185', border: 'rgba(244,63,94,0.3)',   Icon: XCircle       },
};
...
export const StockPage = () => {
  ...
  const [catFilter, setCatFilter] = useState(''); // '' | 'maternal' | 'chronic' | 'general'
  ...
  return (
    <div className="main-content">
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem', marginBottom: '1.5rem' }}>
        <div>
          ...
          <p style={{ color: 'var(--text-secondary)', marginTop: '0.2rem' }}>
            {user?.facility?.name || 'Your facility'} · Tap a quantity to edit
          </p>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          {lowCount > 0 && (
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: '0.35rem', padding: '0.3rem 0.75rem', borderRadius: '9999px', fontSize: '0.78rem', fontWeight: '700', background: 'rgba(244,63,94,0.12)', color: '#fb7185', border: '1px solid rgba(244,63,94,0.3)' }}>
              <AlertTriangle size={13} /> {lowCount} item{lowCount !== 1 ? 's' : ''} need attention
            </span>
          )}
      ...
      <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1.25rem', flexWrap: 'wrap' }}>
        {[{ value: '', label: 'All Categories' }, { value: 'maternal', label: 'Maternal' }, { value: 'chronic', label: 'Chronic' }, { value: 'general', label: 'General / Diagnostic' }].map(f => (
          <button
            key={f.value}
            type="button"
      ...
      {loading ? (
        <div style={{ textAlign: 'center', padding: '4rem', color: 'var(--text-muted)' }}>
          ...
          <div style={{ fontWeight: '700', color: '#f8fafc' }}>Loading inventory…</div>
        </div>
      ) : error ? (
        <div className="card" style={{ textAlign: 'center', padding: '2.5rem', border: '1px solid rgba(244,63,94,0.3)' }}>
          ...
          <button onClick={fetchStock} className="btn btn-primary btn-sm">Retry</button>
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            ...
            return (
              <div key={cat} className="card" style={{ padding: 0, overflow: 'hidden' }}>
                ...
                <div style={{ padding: '0.75rem 1.25rem', borderBottom: '1px solid var(--border-subtle)', display: 'flex', alignItems: 'center', gap: '0.5rem', background: 'rgba(255,255,255,0.02)' }}>
                  ...
                  <span style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>({catItems.length} items)</span>
                  {catItems.some(i => i.status !== 'available') && (
                    <span style={{ marginLeft: 'auto', fontSize: '0.7rem', color: '#fb7185', display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
                      ...
                      {catItems.filter(i => i.status !== 'available').length} low/out
                    </span>
                  )}
                </div>
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
                    <thead>
                      <tr style={{ borderBottom: '1px solid var(--border-subtle)', background: 'rgba(255,255,255,0.02)' }}>
                        {['Item', 'Type', 'Quantity', 'Threshold', 'Status', 'Last Updated'].map(h => (
          ...
          {Object.keys(grouped).length === 0 && (
            <div className="card" style={{ textAlign: 'center', padding: '3rem', border: '1px dashed var(--border-subtle)' }}>
              ...
              <div style={{ color: '#f8fafc', fontWeight: '700' }}>No stock items found</div>
              <div style={{ color: 'var(--text-muted)', fontSize: '0.85rem', marginTop: '0.35rem' }}>Run the seed script to populate the catalog.</div>
            </div>
          )}
        </div>
      )}
